...
class CPU {
   public:
    ...
    // HSIM Integration
    // This constructor is to register callback functions provided by the Bridge class
    CPU(const std::string& config_file, const std::string& output_dir,
            std::function<void(uint64_t)> read_callback, std::function<void(uint64_t)> write_callback)
        : memory_system_(
                config_file, output_dir, read_callback, write_callback),
                clk_(0) {};
